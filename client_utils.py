...
def load_FEMNIST_dataset(
    ...
    Args:
        ...
        target_transform (function): target transform function for dadtaset
    ...
    if target_transform:
        return FEMNIST(
            ...
            data_dir=data_dir,
            transform=transform,
            target_transform=lambda p: to_tensor_transform(target_transform(p)),
        )
    else:
        return FEMNIST(
            mapping=mapping,
...
def test_FEMNIST_with_recall(
    ...
    Args:
        ...
        device (str):  device name onto which perform the computation.
        criterion (Module): generic module describing the loss function.
        source_class (int): class for which recall is computed

    Returns:
        Tuple[float, float, float]: couple of average test loss and average accuracy on the test set.
    """
    correct, total, loss, recall = 0, 0, 0.0, 0.0
    recall_fn = MulticlassRecall(num_classes, average=None)
    net.eval()
    with torch.no_grad():
        num_batches = 0
        for data, labels in tqdm(test_loader):
            ...
            total += labels.size(0)
            correct += (predicted == labels).sum().item()
            recall += recall_fn(predicted, labels)[source_class].item()
            num_batches += 1
...
def get_federated_evaluation_function_with_recall(
    data_dir: Path,
    centralized_mapping: Path,
    device: str,
    batch_size: int,
    ...
    def federated_evaluation_function(
        ...
        loss, acc, recall = test_FEMNIST_with_recall(
            net=net,
            test_loader=valid_loader,
            device=device,
            criterion=criterion,
        ...
        return loss, {"accuracy": acc, 'recall': recall}
